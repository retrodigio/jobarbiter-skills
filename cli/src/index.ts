...
program
	.command("analyze")
	.description("Analyze AI session transcripts and submit work reports")
	.option("--dry-run", "Analyze but don't submit — show report instead")
	.option("--session <path>", "Analyze a specific session/transcript file")
	.option("--auto", "Auto mode — used by observer hooks (silent, non-interactive)")
	.option("--since <hours>", "Analyze sessions from last N hours", parseFloat)
	.option("--retry", "Retry previously queued (failed) submissions")
	.action(async (opts) => {
		try {
			// Auto mode: silent fire-and-forget from observer hooks
			if (opts.auto) {
			...
			if (opts.retry) {
				const { retryQueuedReports } = await import("./lib/report-submitter.js");
				const result = await retryQueuedReports();
				if (result.succeeded + result.failed + result.remaining === 0) {
					console.log("No queued reports to retry.");
				} else {
					console.log(`\nRetry results: ${result.succeeded} succeeded, ${result.failed} dropped, ${result.remaining} still queued.\n`);
				}
				output(result);
				process.exit(0);
			}
			...
			if (opts.session) {
				const report = analyzeFile(opts.session);
				if (!report) {
					error("Could not parse transcript file.");
					process.exit(1);
				}
				...
				if (!opts.dryRun) {
					const { submitWorkReport } = await import("./lib/report-submitter.js");
					const result = await submitWorkReport(report);
					if (result.success) {
						success(`Report submitted (ID: ${result.reportId || "n/a"})`);
					} else {
						error(`Submission failed: ${result.error}${result.queued ? " (queued for retry)" : ""}`);
					}
				} else {
			...
			console.log(`\n🔍 Analyzing sessions from the last ${sinceHours} hours...\n`);

			const result = await analyzeRecent({ dryRun: opts.dryRun, since });
			...
			if (opts.dryRun) {
				console.log(`Found ${result.reports.length} session(s). (dry run — not submitted)\n`);
			} else {
				console.log(`Analyzed ${result.reports.length} session(s), submitted ${result.submitted}.\n`);
			}
